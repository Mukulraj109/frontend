...
const EditProfile = () => {
    ...
    const handelSubmit = (e) =>{
        ...
        if(bio.length>bioLimit){
            return toast.error(`Bio should not be more than ${bioLimit} characters long`);
        }
                    ...
                    <div className="flex flex-col lg:flex-row items-start py-10 gap-8 lg:gap-10">
                        <div className="max-lg:center mb-5">
                            <label htmlFor="uploadImg" id="profileImgLabel" className="relative block w-48 h-48 bg-grey rounded-full overflow-hidden">
                                ...
                                <img ref={profileImageEle} src={profile_img} alt="Profile" />
                            </label>
                            <input type="file" id="uploadImg" accept=".jpeg,.png,.jpg" hidden  onChange={handelImagePreview}/>
                            <button onClick = {handelImageUpload} className="btn-light mt-5 max-lg:center lg:w-full px-10">Upload</button>
                        </div>
                        <div className="w-full">
                            ...
                            <textarea  onChange={handelCharacterChange} name= "bio" maxLength={bioLimit} defaultValue={bio} className="input-box h-64 lg:h-40 resize-none leading-7 mt-5 pl-5" placeholder="Bio">
                            </textarea>
                            <p className="mt-1 text-dark-grey">{characterLeft}Character left</p>
                            <p className="my-6 text-dark-grey">Add your social handel below</p>

                            <div className="md:grid md:grid-cols-2 gap-x-6">
                                {
                                    Object.keys(social_links).map((key,i)=>{
                                        let link = social_links[key];
                                        return<InputBox key={i} name = {key} type =" text" value = {link} placeholder="https://" icon={"fi "+(key!='website' ? "fi-brands-" + key :
                                            "fi-rr-globe")}/>
                                    })
                                }

                            </div>
                            <button onClick={handelSubmit} className="btn-dark w-auto px-10" type="submit">
                                Update
                            </button>
                        </div>
                    </div>
